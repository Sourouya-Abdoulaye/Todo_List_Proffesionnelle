...
<a href="/create" class="w-3  ">
    <button type="button" class="bg-green-500 m-4 w-36 rounded-md p-1 hover:bg-green-600 transition ">Ajouter une Tache</button>
</a>
<div class="overflow-x-auto max-w-full px-4">
    <table class="min-w-full border border-gray-200 shadow-md rounded-lg overflow-hidden">
        <thead class="bg-green-500 text-white">
            <tr>
                ...
                <th class="p-3 text-left text-sm font-medium uppercase">
                    Status :
                    <select name="status" id="status" class="bg-transparent pl-0 border-none outline-none cursor-pointer focus:ring-0"
                        onchange="window.location.href='/'+(this.value ? '?status='+this.value : '')">
                        
                        <option class="text-black" value="" {{ request('status') == '' ? 'selected' : '' }}>Toutes</option>
                        <option class="text-black" value="1" {{ request('status') == '1' ? 'selected' : '' }}>Terminé</option>
                        <option class="text-black" value="0" {{ request('status') == '0' ? 'selected' : '' }}>En attente</option>
                    </select>
                </th>
        ...
        @else
            ...
            <tr><td colspan="6" class="text-center text-2xl p-8">Aucune tâche Enregistrer</td></tr>
        @endif

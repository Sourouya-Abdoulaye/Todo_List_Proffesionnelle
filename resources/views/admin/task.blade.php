...
<div class="max-w-4xl mx-auto mb-6 px-4">
    <input type="text" id="tache_search" placeholder="Rechercher une tâche..." class="w-full p-3 rounded-lg border border-gray-300 focus:outline-none
...
<a href="/create" class="w-3  ">
    <button type="button" class="bg-green-500 m-4 w-36 rounded-md p-1 hover:bg-green-600 transition ">Ajouter une
        Tache</button>
</a>
    ...
    <table class="min-w-full border border-gray-200 shadow-md rounded-lg overflow-hidden">
        <thead class="bg-green-500 text-white">
            <tr>
                ...
                <th class="p-3 text-left text-sm font-medium uppercase">
                    Status :
                    <select name="status" id="status"
                        class="bg-transparent pl-0 border-none outline-none cursor-pointer focus:ring-0"
                        onchange="window.location.href='/admin/tasks'+(this.value ? '?status='+this.value : '')">

                        <option class="text-black" value="" {{ request('status')=='' ? 'selected' : '' }}>Toutes
                        </option>
                        <option class="text-black" value="1" {{ request('status')=='1' ? 'selected' : '' }}>Terminé
                        </option>
                        <option class="text-black" value="0" {{ request('status')=='0' ? 'selected' : '' }}>En attente
                        </option>
                    </select>
                </th>
        ...
        <tbody id="table-body" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr class="hover:bg-green-100 dark:hover:bg-green-700 transition">
                ...
                <td class="p-3 text-sm">{{ \Illuminate\Support\Str::limit($task->title, 20) }} </td>
                ...
                <td class="p-3 flex flex-wrap gap-2">
                    <a href="/{{$task->id}}/show"
                        class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded transition text-sm">Show</a>
                    <a href="/{{$task->id}}/edit"
                        class="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded transition text-sm">Update</a>
                    <form action="/{{$task->id}}/delete" method="POST"
                        onsubmit="return confirm('Supprimer cet élément ?');">
                        @csrf
                        @method('DELETE')
                        <button
                            class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white rounded transition text-sm">Delete</button>
                    </form>
                </td>
            </tr>
            @endif
            ...
            <tr class="hover:bg-green-100 dark:hover:bg-green-700 transition">
                ...
                <td class="p-3 flex flex-wrap gap-2">
                    <a href="/{{$task->id}}/show"
                        class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded transition text-sm">Show</a>
                    <a href="/{{$task->id}}/edit" onclick="return confirm('Voulez vous mettre à jour ?');"
                        class="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded transition text-sm">Update</a>
                    <form action="/{{$task->id}}/delete" method="POST"
                        onsubmit="return confirm('Supprimer cet élément ?');">
                        @csrf
            ...
            @endforeach
            @else
            <!-- s'il n'existe pas de tache -->
            <tr>
                <td colspan="6" class="text-center text-2xl p-8">Aucune tâche Enregistrer</td>
            </tr>
            @endif
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    data.forEach(task => {
                        ...
                        <tr class="hover:bg-green-100 dark:hover:bg-green-700 transition">
                            <td class="p-3 text-sm">${task.id}</td>
                            <td class="p-3 text-sm"> ${task.title.substring(0, 20)}...</td>
